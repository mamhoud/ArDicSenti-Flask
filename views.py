...
def contr_step_2(request):
    radiobutton = request.POST['addword']
    word = request.POST['data']
    row = search(word)

    if radiobutton == "Postive":
        modifyPos(row, 1)
        return render(request, 'app/Positive_added.html')
    elif radiobutton == "Negative":
        modifyNeg(row, 1)
        return render(request, 'app/Negative_added.html')
    elif radiobutton == "Misleading":
        modifyMis(row, 1)
        return render(request, 'app/Missleading_added.html')
...
def addword(request):
    ...
    if word == "" and radiobutton == "":
        return render(request,'app/Nothing_added.html')
    else:
        root=stemer(word)
        row = search(root)
        if radiobutton == "po":
...
def download_file(request):
    ...
    if len(value) == 3:
        ...
        if value[0] == 'postive' and value[1] == 'negative' and value[2] == "miss" and pos_percentage != "" and neg_percentage != "" and mis_percentage != "":
